...
export default function X6CanvasBlock({ node, graph }: X6CanvasBlockProps) {
  ...
  const handleAddBranch = (e: React.MouseEvent) => {
    e.stopPropagation();
    e.preventDefault();
    const event = new CustomEvent('x6-add-branch', {
  ...
  // When collapsed, show chevron + dashed line to the left of the node
  return (
    <div
      className={`x6-block-outer ${isCollapsed && hasChildren ? 'has-collapsed-indicator' : ''}`}
      style={{
        width: '100%',
        height: '100%',
        position: 'relative',
    ...
    >
      {/* Collapsed indicator: dashed line + chevron button to the LEFT */}
        ...
        >
          ...
          <div
            style={{
              width: 24,
              height: 0,
              borderTop: `3px dashed ${branchColor}`,
              opacity: 0.7,
            }}
          />
          {/* Chevron expand button */}
          <button
            className="x6-expand-chevron"
      ...
      {/* Hover "+" button to add a branch — appears on right edge */}
      {!block.isEditing && (
        <button
          className="x6-add-branch-btn"
      ...
      >
        ...
        <div style={{ position: 'relative', zIndex: 1, padding: '6px 22px', height: '100%', display: 'flex', flexDirection: 'column', alignItems: block.text.length <= 50 ? 'center' : 'flex-start', justifyContent: 'center', textAlign: block.text.length <= 50 ? 'center' : 'left' }}>
          ...
          {!isCollapsed && !block.isEditing && (
            <div
              className="block-content block-fade-in"
          ...
          {!isCollapsed && block.isEditing && (
            <div
              className="block-content editing block-fade-in"
          ...
          {isCollapsed && (
            <div className="block-content collapsed-preview block-fade-in">
